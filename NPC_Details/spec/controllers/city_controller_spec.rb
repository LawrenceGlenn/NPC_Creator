  ...
  describe CitiesController do
    context "#parentOldEnough?" do
      context "parent and child same age" do
       it "returns false" do
        ...
        parent = create(:npc_joe, race: human)
        child = create(:npc_joe, race: human)
      ...
      context "parent 1 year older than child" do
       it "returns false" do
        human = create(:race_human)
        parent = create(:npc_joe, race: human, age: 40)
        child = create(:npc_joe, race: human, age: 39)
        expect(controller.send(:parentOldEnough?, parent, child)).to  eq(false)
        end
      end
      context "parent 1 year younger than child" do
       it "returns false" do
        human = create(:race_human)
        parent = create(:npc_joe, race: human, age: 38)
        child = create(:npc_joe, race: human, age: 39)
        expect(controller.send(:parentOldEnough?, parent, child)).to  eq(false)
        end
      end
      context "human parent 16 year older than child" do
       it "returns true" do
        human = create(:race_human)
        parent = create(:npc_joe, race: human, age: 56)
        child = create(:npc_joe, race: human, age: 40)
      ...
      context "human parent 16 year younger than child" do
       it "returns false" do
        human = create(:race_human)
        parent = create(:npc_joe, race: human, age: 24)
        child = create(:npc_joe, race: human, age: 40)
        expect(controller.send(:parentOldEnough?, parent, child)).to  eq(false)
        end
      end
      context "human parent 16 year older than dwarf child" do
       it "returns true" do
        ...
        parent = create(:npc_joe, race: human, age: 61)
        child = create(:npc_joe, race: dwarf, age: 45)
        expect(controller.send(:parentOldEnough?, parent, child)).to  eq(true)
        end
      end
      context "human parent 11 year younger than dwarf child" do
       it "returns true" do
        human = create(:race_human)
        dwarf = create(:race_dwarf)
        parent = create(:npc_joe, race: human, age: 32)
        child = create(:npc_joe, race: dwarf, age: 43)
        expect(controller.send(:parentOldEnough?, parent, child)).to  eq(true)
        end
      end
      context "human parent 12 year younger than dwarf child" do
       it "returns true" do
        human = create(:race_human)
        dwarf = create(:race_dwarf)
        parent = create(:npc_joe, race: human, age: 31)
        child = create(:npc_joe, race: dwarf, age: 43)
        expect(controller.send(:parentOldEnough?, parent, child)).to  eq(false)
        end
      end
      context "human parent 82 year younger than elf child" do
       it "returns true" do
        ...
        elf = create(:race_elf)
        parent = create(:npc_joe, race: human, age: 32)
        child = create(:npc_joe, race: elf, age: 114)
        expect(controller.send(:parentOldEnough?, parent, child)).to  eq(true)
        end
      end
    ...
    context "#tooCloseToPreviousChildsAge?" do
      context "child and previous child same age" do
       it "returns true" do
        human = create(:race_human)
        previousChild = create(:npc_joe, race: human)
        child = create(:npc_joe, race: human)
      ...
      context "child is older than previous child" do
       it "returns false" do
        human = create(:race_human)
        previousChild = create(:npc_joe, race: human, age: 30)
        child = create(:npc_joe, race: human, age: 31)
      ...
      context "child is younger than previous child" do
       it "returns false" do
        human = create(:race_human)
        previousChild = create(:npc_joe, race: human, age: 30)
        child = create(:npc_joe, race: human, age: 29)
        expect(controller.send(:tooCloseToPreviousChildsAge?, child, previousChild, false)).to  eq(false)
        end
      end
    end
